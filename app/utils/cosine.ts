...
/** 캐시된 AI 전형 벡터 목록 (메모리) */
let featureVectorCache: Array<[number, number, number, number, number]> = [];
let cacheLoaded = false;

/**
 * 두 5차원 벡터 간 코사인 유사도를 계산합니다.
 * 반환값: [-1, 1] (1에 가까울수록 유사)
 */
...
export function updateFeatureVectorCache(entries: FeatureVectorEntry[]): void {
  featureVectorCache = entries
    .filter((e) => e.vector && e.vector.length === 5)
    .map((e) => e.vector as [number, number, number, number, number]);
  cacheLoaded = true;
 ...
 *
 * @param vector - 분석 대상 텍스트의 5차원 feature vector
...
export function computeCompositeAIScore(params: {
  ...
  const W4 = 0.10; // TTR weight (inverted)
  const W5 = 0.10; // entropy weight (inverted)
